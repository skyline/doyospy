...
//    public static final Set<Character> RE_KEYS = CollUtil.newHashSet(new Character[]{'$', '(', ')', '*', '+', '.', '[', ']', '?', '\\', '^', '{', '}', '|'});
//
//    public ReUtil() {
//    }
//
...
//        return null != pattern && !StrUtil.isEmpty(content) ? pattern.matcher(content).replaceAll("") : StrUtil.str(content);
//    }
//
...
//                return StrUtil.str(content);
//            } else {
//                Set<String> varNums = (Set)findAll((Pattern)PatternPool.GROUP_VAR, replacementTemplate, 1, new TreeSet(LengthComparator.INSTANCE.reversed()));
